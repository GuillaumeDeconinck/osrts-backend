...
const checkAndUpdateTagHook = context => new Promise((resolve, reject) => {
  ...
  if (newRunner.tag && newRunner.tag && newRunner.tag.num) {
    ...
    runnersService.get(context.id).then((oldRunner) => {
      ...
      if (oldRunner.tag
        && oldRunner.tag.num === newRunner.tag.num
        && oldRunner.tag.color === newRunner.tag.color
      ) {
        resolve(context);
      } else {
        tagsService.find(
          { query: { num: newRunner.tag.num, color: newRunner.tag.color } },
        ).then((tag) => {
          if (tag.total !== 1) {
            reject(new Error('Ce tag n\'existe pas !'));
          } else if (tag.data[0].assigned === true) {
            reject(new Error('Déjà un coureur avec ce tag !'));
          } else if (oldRunner.tag && oldRunner.tag.num) {
            // Otherwise update the two tags
            const promiseOldTag = tagsService.patch(
              null,
              { assigned: false },
              { query: { num: oldRunner.tag.num, color: oldRunner.tag.color } },
            );
            const promiseNewTag = tagsService.patch(
              null,
              { assigned: true },
              { query: { num: newRunner.tag.num, color: newRunner.tag.color } },
            );
            Q.allSettled([promiseOldTag, promiseNewTag]).then((results) => {
              results.forEach((tag2) => {
